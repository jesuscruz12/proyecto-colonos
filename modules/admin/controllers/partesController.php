<?php
// C:\xampp\htdocs\qacrmtaktik\modules\admin\controllers\partesController.php

class partesController extends adminController
{
    /** @var partesModel */
    private $_m;
    ...
    {
        $this->_view->menu = 'partes';
        $this->_view->setJs(['index']); // /modules/admin/views/partes/js/index.js
    ...
    public function recursos_meta()
    {
        try {
            ...
            $this->json([
                'ok' => true,
                'data' => [
                    'clientes' => $this->_m->clientesList($eid),
                    'unidades' => $this->_m->unidadesList($eid),
                ]
            ]);
        } catch(Throwable $e) {
    ...
    {
        $id = (int)($_GET['id'] ?? 0);
        $r  = $this->_m->getParte($this->empresaId(), $id);
    ...
    {
        try {
            $id = $this->_m->saveParte($this->empresaId(), $_POST);
    ...
    public function delete()
    {
        try {
            ...
            $this->_m->deleteParte($this->empresaId(), $id);
